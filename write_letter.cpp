...
{
  std::ofstream ofs;
  ofs.open("/home/cz2398/PR2write.txt", std::ofstream::out | std::ofstream::trunc);
  ofs.close();
}
void readFile(std::vector<float> &points, int spec_line)
{
  points.clear();
  std::ifstream infile;
  infile.open ("/home/cz2398/letters.txt");
  char buffer[1025];
...
{
  std::ofstream out("/home/cz2398/PR2write.txt", std::ios::app);
  if(out.good())
  {
	out << x-basex <<"\n";
	out << y-basey <<"\n";
